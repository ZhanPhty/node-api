  ...
  $.fn.splitter = function(args) {
    ...
    return this.each(function() {
      ...
      function dimSum(jq, dims) {
        ...
        var sum = 0
        for (var i = 1; i < arguments.length; i++) sum += Math.max(parseInt(jq.css(arguments[i])) || 0, 0)
        return sum
      }
      ...
      splitter._PBF = $.boxModel
        ? dimSum(splitter, 'border' + opts.side3 + 'Width', 'border' + opts.side4 + 'Width')
        : 0
      splitter._PBA = $.boxModel
        ? dimSum(splitter, 'border' + opts.side1 + 'Width', 'border' + opts.side2 + 'Width')
        : 0
      A._pane = opts.side1
      B._pane = opts.side2
      $.each([A, B], function() {
...
function setUpController() {
  ...
  _player.createPluginHost = function(settings) {
    ...
    var host = $('<div id=' + settings.id + '></div>').appendTo('#interfaceControlFrameHostContainer')
    ...
    headerLink
      .click($axure.utils.curry(interfaceControlHeaderButton_click, settings.id))
      .wrap('<li id="' + settings.id + 'Btn">')

    if (
      (settings.id == 'feedbackHost' || settings.id == 'feedbackContainer') &&
      pluginIds[pluginIds.length - 1] == 'debugHost'
    )
      headerLink.parent().insertBefore('#debugHostBtn')
...
;(function() {
  ...
  if (!$axure.browser.isEdge) {
    ...
    CHROME_5_LOCAL = chromeMatch && Number(chromeMatch[1]) >= 5 && location.href.indexOf('file://') >= 0
  }
