...
;(function() {
  ...
  $(window.document).ready(function() {
    ...
    $axure.page.bind('load.sitemap', function() {
      ...
      currentNodeUrl = decodedPageLoc.substr(
        decodedPageLoc.lastIndexOf('/') ? decodedPageLoc.lastIndexOf('/') + 1 : 0
      )
      currentPlayerLoc = $(location)
        .attr('href')
        .split('#')[0]
        .split('?')[0]
      ...
      $('.sitemapPageLink[nodeUrl="' + currentNodeUrl + '"]')
        .parent()
        .parent()
        .addClass('sitemapHighlight')

      var pageName = $axure.page.pageName
      $('.pageNameHeader').html(pageName)

      $('#sitemapLinksPageName').html(
        $('.sitemapHighlight > .sitemapPageLinkContainer > .sitemapPageLink > .sitemapPageName').html()
      )
    ...
    $adaptiveViewsContainer.append(
      '<div class="adaptiveViewOption" val="auto"><div class="adaptiveCheckboxDiv checkedAdaptive"></div>Auto</div>'
    )
    ...
    if (typeof $axure.document.defaultAdaptiveView.name != 'undefined') {
      ...
      $adaptiveViewsContainer.append(
        '<div class="adaptiveViewOption currentAdaptiveView" val="default"><div class="adaptiveCheckboxDiv"></div>' +
          defaultViewName +
          '</div>'
      )
    ...
    for (var viewIndex = 0; viewIndex < $axure.document.adaptiveViews.length; viewIndex++) {
      ...
      $adaptiveViewsContainer.append(
        '<div class="adaptiveViewOption" val="' +
          currView.id +
          '"><div class="adaptiveCheckboxDiv"></div>' +
          viewString +
          '</div>'
      )
  ...
  function withoutSitemapRadio_click() {
    ...
    $('input[value="withmap"]')
      .parent()
      .removeClass('sitemapRadioSelected')
  ...
  function sitemapUrlOptions_change() {
    ...
    newHash = $(this).is(':checked')
      ? setHashStringVar(currLinkHash, varName, defVal)
      : deleteHashStringVar(currLinkHash, varName)
  ...
  function sitemapUrlViewSelect_change() {
    ...
    newHash =
      selectedVal == 'auto'
        ? deleteHashStringVar(currLinkHash, ADAPTIVE_VIEW_VAR_NAME)
        : setHashStringVar(currLinkHash, ADAPTIVE_VIEW_VAR_NAME, selectedVal)
  ...
  function generateSitemap() {
    ...
    treeUl +=
      "<div><label><input type='radio' name='sitemapToggle' value='withoutmap'/>Without Sidebar</label></div>"
    treeUl +=
      "<div style='margin-top:10px;'><label><input type='radio' name='sitemapToggle' value='withmap'/>With Sidebar</label>"

    treeUl += "<div id='sitemapOptionsDiv'>"
    treeUl +=
      "<div class='sitemapUrlOption'><label><input type='checkbox' id='minimizeBox' />Minimize sidebar</label></div>"
    treeUl +=
      "<div class='sitemapUrlOption'><label><input type='checkbox' id='collapseBox' />Pages closed</label></div>"
    if ($axure.document.configuration.showAnnotations == true) {
      treeUl +=
        "<div class='sitemapUrlOption'><label><input type='checkbox' id='footnotesBox' />Hide footnotes</label></div>"
    }

    treeUl +=
      "<div class='sitemapUrlOption'><label><input type='checkbox' id='highlightBox' />Highlight interactive elements</label></div>"

    if ($axure.document.configuration.enabledViewIds.length > 0) {
      treeUl +=
        "<div id='viewSelectDiv' class='sitemapUrlOption'><label>View: <select id='viewSelect'></select></label></div>"
    }
...
;(function() {
  ...
  $(window.document).ready(function() {
    ...
    $('#footnotesButton')
      .click(footnotes_click)
      .addClass('sitemapToolbarButtonSelected')
  ...
  function generatePageNotes() {
    ...
    pageNotesUi +=
      "<a id='notesPreviousButton' title='Previous Page' class='sitemapToolbarButton prevPageButton'></a>"
    pageNotesUi +=
      "<a id='notesNextButton' title='Next Page' class='sitemapToolbarButton nextPageButton'></a>"
...
;(function() {
    ...
    var processMessages = function(message, data) {
      if (message == 'globalVariableValues') {
        ...
        for (var key in data) {
          ...
          $('#variablesDiv').append(
            '<div class="variableDiv"><span class="variableName">' + key + '</span><br/>' + value + '</div>'
          )
        }
        ...
        if (currentStack.length == 0) {
          ...
          for (var i = finishedStack.length - 1; i >= 0; i--) {
            ...
            if (i == finishedStack.length - 1)
              $('#traceDiv')
                .children()
                .first()
                .addClass('lastAxEvent')
          }
          finishedStack = []
        }
  ...
  function generateDebug() {
    ...
    pageNotesUi +=
      "<div id='variablesClearLinkContainer' class='debugLinksContainer'><a id='variablesClearLink' title='Reset Variables'>Reset Variables</a></div>"
    pageNotesUi += "<div id='variablesDiv'></div></div>"
    pageNotesUi += "<div class='dottedDivider'></div>"
    pageNotesUi += "<div id='traceContainer'>"
    pageNotesUi +=
      "<div id='traceClearLinkContainer' class='debugLinksContainer'><a id='traceClearLink' title='Clear Trace'>Clear Trace</a></div>"
    pageNotesUi +=
      "<div id='traceEmptyState' class='emptyStateContainer'><div class='emptyStateTitle'>No interactions in the trace.</div><div class='emptyStateContent'>Triggered interactions will appear here.</div><div class='dottedDivider'></div></div>"
