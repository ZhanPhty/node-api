...
;(function() {
    ...
    if (query == '*') {
      ...
      if (firstString.charAt(0) == '@') {
        ...
        if (firstString == 'label') {
          _axQueryObject.filterFunctions[0] = function(diagramObject) {
            return (
              (queryArg instanceof Array && queryArg.indexOf(diagramObject.label) > 0) ||
          ...
          _axQueryObject.filterFunctions[0] = function(diagramObject, elementId) {
            return (queryArg instanceof Array && queryArg.indexOf(elementId) > 0) || elementId == queryArg
          }
        }
      }
    }
  ...
  $ax.public.fn.filter = function(query, queryArg) {
    ...
    for (var i = 0; i < this.query.filterFunctions.length; i++)
      returnVal.query.filterFunctions[i + offset] = this.query.filterFunctions[i]
  ...
  $ax.public.fn.getChildren = function(deep, ignoreUnplaced) {
    ...
    var getChildren = function(elementId) {
      ...
      if (isRepeater || isDynamicPanel || isLayer || isMaster || isMenu || isTreeNode || isTable) {
        // || isCompoundVector) {
        ...
        if (isRepeater) {
          ...
          for (var itemIndex = 0; itemIndex < itemIds.length; itemIndex++)
            parent = parent.add($jobj($ax.repeater.createElementId(elementId, itemIds[itemIndex])))
        ...
        for (var childIndex = 0; childIndex < children.length; childIndex++) {
          ...
          // Ignore Unplaced
          if (ignoreUnplaced && $ax.visibility.isScriptIdLimbo($ax.repeater.getScriptIdFromElementId(id)))
            continue
          childrenIds.push(id)
        }
  ...
  $ax.annotation.InitializeAnnotations = function(query) {
    ...
    query.each(function(dObj, elementId) {
      ...
      if (dObj.type == 'hyperlink') {
        ...
        if ($ax.document.configuration.useLabels) {
          ...
          $('#' + elementId + '_ann').append(
            "<div id='" + elementId + "Note' class='annnotelabel'>" + label + '</div>'
          )
        } else {
      ...
      $('#' + elementId + 'Note.annnoteimage').append(
        "<div class='annnoteline'></div><div class='annnoteline'></div><div class='annnoteline'></div>"
      )
    })
  }
...
$axure.internal(function($ax) {
  ...
  for (var jqueryFunctionIndex = 0; jqueryFunctionIndex < _jQueryFunctionsToAdd.length; jqueryFunctionIndex++)
  ...
  for (var jqueryEventIndex = 0; jqueryEventIndex < _jQueryEventFunctionsToAdd.length; jqueryEventIndex++)
  ...
  $ax.public.fn.setOpacity = function(opacity, easing, duration) {
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if (duration == 0 || easing == 'none') {
        ...
        query.animate(
          { opacity: opacity },
          { duration: duration, easing: easing, queue: false, complete: onComplete }
        )
    }
  }
  ...
  $ax.public.fn.circularMoveAndRotate = function(
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if (doRotation)
        $ax.move.rotate(elementId, degreeChange, options.easing, options.duration, false, true, function() {
          $ax.dynamicPanelManager.fitParentPanel(elementId)
        })
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if (isDynamicPanel) {
        ...
        if (
          ...
          if (loc) {
            if (loc[0] != 0 && !$ax.dynamicPanelManager.isPercentWidthPanel(obj))
              newLocationAndSizeCss['margin-left'] =
                '+=' + (Number(newLocationAndSizeCss['margin-left'].substr(2)) + loc[0])
            if (loc[1] != 0)
              newLocationAndSizeCss['margin-top'] =
                '+=' + (Number(newLocationAndSizeCss['margin-top'].substr(2)) + loc[1])
          }
        }
      ...
      if (children && children.length !== 0) {
        ...
        children.each(function(i, child) {
          ...
          var differentSizedImage =
            childSizingObj.width - oldWidth != 0 || childSizingObj.height - oldHeight != 0
          if ((differentSizedImage || isConnector) && child.tagName == 'IMG') {
            //oldwidth is zero for connectors
            var widthOffset = oldWidth
              ? ((childSizingObj.width - oldWidth) * newLocationAndSizeCss.width) / oldWidth
              : childSizingObj.width
            var heightOffset = oldHeight
              ? ((childSizingObj.height - oldHeight) * newLocationAndSizeCss.height) / oldHeight
            ...
            if (animationObj.resizeMatrixFunction) {
              $(animationObj.obj).css(
                $ax.public.fn.setTransformHowever(
                  animationObj.resizeMatrixFunction(animationObj.width, animationObj.height)
                )
              )
            } else {
  ...
  $ax.public.fn.text = function() {
    if (arguments[0] == undefined) {
      ...
      for (var index = 0; index < elementIds.length; index++) {
        ...
        if ($ax.public.fn.IsTextBox(widgetType) || $ax.public.fn.IsTextArea(widgetType)) {
          //For non rtf
  ...
  var getWidgetText = function(id) {
    ...
    if (
      idQuery.is('input') &&
      ($ax.public.fn.IsCheckBox(idQuery.attr('type')) || idQuery.attr('type') == 'radio')
    ) {
      idQuery = idQuery
        .parent()
        .find('label')
  ...
  var isSoloBr = function(html) {
    ...
    return (
      spanChildren && spanChildren.length == 1 && spanChildren.is('br') && spanChildren.text().trim() == ''
    )
  }
  ...
  $ax.public.fn.value = function() {
    if (arguments[0] == undefined) {
      ...
      if ($ax.public.fn.IsComboBox(widgetType) || $ax.public.fn.IsListBox(widgetType)) {
      ...
      for (var index = 0; index < elementIds.length; index++) {
        ...
        if ($ax.public.fn.IsCheckBox(widgetType) || $ax.public.fn.IsRadioButton(widgetType)) {
          //for radio/checkboxes
          if (arguments[0] == true) {
  ...
  $ax.public.fn.enabled = function() {
    if (arguments[0] == undefined) {
      ...
      for (var index = 0; index < elementIds.length; index++) {
        ...
        if (OS_MAC && WEBKIT && $ax.public.fn.IsComboBox(widgetType))
          jobj.css('color', enabled ? '' : 'grayText')
  ...
  var _bodyToWorld = ($axure.fn.bodyToWorld = function(x, from) {
    ...
    var offset =
      Number(body.css('left').replace('px', '')) + Math.max(0, ($(window).width() - body.width()) / 2)
    if (from) offset *= -1
    return x + offset
  })
  ...
  var _getChildLoc = function(children, vert, high, dim, root, path, itemId) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if ($ax.public.fn.IsReferenceDiagramObject(childObj.type)) {
        ...
        var childProp = _getChildLoc(
          $ax.pageData.masters[$obj(childId).masterId].diagram.objects,
          vert,
          high,
          dim,
          false,
...
$axure.internal(function($ax) {
  ...
  ;(function() {
        ...
        for (var i = 0; i < objs.length; i++) {
          ...
          if ($ax.public.fn.IsDynamicPanel(obj.type)) {
            for (var j = 0; j < obj.diagrams.length; j++)
              mapIdsToRepeaterAndLayer(path, obj.diagrams[j].objects, repeaterId)
    ...
    var _getElementsIdFromEventAndScriptId = function(eventInfo, scriptId) {
      ...
      if (!itemId) {
        itemId =
          eventInfo && eventInfo.targetElement && $ax.repeater.getItemIdFromElementId(eventInfo.targetElement)
          ...
          (!eventInfo ||
            parentRepeater ==
              $ax.getParentRepeaterFromScriptId(
                $ax.repeater.getScriptIdFromElementId(target ? eventInfo.targetElement : eventInfo.srcElement)
              ))
        ) {
    ...
    var _getSrcElementIdFromEvent = function(event) {
      ...
      while (
        currentQuery &&
        currentQuery.length &&
        (!$obj(currentQuery.attr('id')) || $jobj(currentQuery.attr('id')).hasClass('text'))
      ) {
    ...
    var _getWidgetInfo = function(elementId) {
      ...
      var repeaterId = $ax.getParentRepeaterFromScriptId(scriptId)
      if (repeaterId)
        widget.repeater = $ax.public.fn.IsRepeater(obj.type) ? widget : _getWidgetInfo(repeaterId)
    ...
    $ax.GetTextPanelId = function(id, create) {
      ...
      if (!panelDiv) {
        ...
        var newDiv = $(
          '<div id="' +
            newId +
            '" class="text" style="visibility: inherit; position: absolute"><p><span></span></p></div>'
        )
    ...
    $ax.addItemIdToRepeater = function(itemId, repeaterId) {
      ...
      for (var i = 0; i < scriptIds.length; i++)
        elementIdToObject[$ax.repeater.createElementId(scriptIds[i], itemId)] = $ax.getObjectFromScriptId(
          scriptIds[i]
        )
    }
      ...
      for (var i = 0; i < _scriptIds.length; i++) {
        ...
        if (repeaterId && repeaterId != scriptId) {
          ...
          for (var j = 0; j < itemIds.length; j++)
            elementIds[elementIds.length] = $ax.repeater.createElementId(scriptId, itemIds[j])
    ...
    $ax.getRdoParentFromElementId = function(elementId) {
      ...
      if ($ax.getParentRepeaterFromScriptId(rdoId))
        rdoId = $ax.repeater.createElementId(rdoId, $ax.repeater.getItemIdFromElementId(elementId))
      return rdoId
    }
