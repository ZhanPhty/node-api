...
exports.list = async (ctx, next) => {
  ...
  await ctx.mongo.select.Category.paginate({},
    {
      sort: { created: -1 },
      page,
      limit,
...
exports.create = async (ctx, next) => {
  ...
  let find = await ctx.mongo.select.Category.findOne({ name: ctx.request.body.name }).exec()
  ...
  if (find) {
    ...
    const result = await ctx.mongo.select.Category.create(create)
...
exports.del = async (ctx, next) => {
  ...
  const { id } = ctx.params
  const result = await ctx.mongo.select.Category.deleteMany({
    _id: id
  })
